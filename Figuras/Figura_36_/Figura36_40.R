...
# Convertendo sf object para Spatial
plot_data_sp <- as(plot_data_sf, "Spatial")
sedes_municipais_sp <- as(sedes.municipais_sf, "Spatial")



# Function to perform IDW interpolation with a specific idp value
interpolate_idw <- function(variable, idp_value, plot_data_sp, sedes_municipais_sp) {
  # Create the gstat object for the variable of interest
  gstat_obj <- gstat::gstat(formula = as.formula(paste(variable, "~ 1")),
                            locations = plot_data_sp,
                            nmax = 15,    # Maximum number of neighbors to consider
                            set = list(idp = idp_value))
  
  # Perform IDW interpolation
  interpolated <- predict(gstat_obj, newdata = sedes_municipais_sp)
  
  # Extract coordinates from the Spatial object
  coords <- coordinates(interpolated)
  
  # Convert to data frame, add coordinates, and rename the prediction column
  result_df <- as.data.frame(interpolated)
  result_df$x <- coords[, 1]
  result_df$y <- coords[, 2]
  colnames(result_df)[1] <- paste0(variable, "_pred")  # Rename the prediction column
  
  return(result_df)
}

# List of variables for interpolation
variables <- c("rainfall", "idf_a", "idf_b", "idf_c", "idf_d")

# Create a list to store the results
results <- list()

# Iterate over each unique Tr_value
unique_tr_values <- unique(plot_data_sf$Tr_values)

for(tr_value in unique_tr_values) {
  # Define a specific idp value for the current Tr_value
  idp_value <- 2.5  # or some other adjustable value
  
  # Filter the data for the specific Tr_value
  plot_data_sp_filtered <- plot_data_sp[plot_data_sf$Tr_values == tr_value, ]
  
  # Interpolate all variables and store them in a list
  interpolated_data <- lapply(variables, function(var) {
    interpolate_idw(var, idp_value, plot_data_sp_filtered, sedes_municipais_sp)
  })
  
  # Combine the results into a single data frame, while keeping coordinates only once
  combined_results <- do.call(cbind, lapply(interpolated_data, function(df) df[, 1]))
  colnames(combined_results) <- paste0(variables, "_pred")  # Naming each column appropriately
  combined_results <- cbind(combined_results, interpolated_data[[1]][, c("x", "y")])
  combined_results$Tr_value <- tr_value
  
  # Store the results
  results[[as.character(tr_value)]] <- combined_results
}
...
final_results_sf <- st_as_sf(final_results, coords = c("x", "y"), crs = 4326)

# View the final sf object
print(final_results_sf)
...
#terminou rascunho
interpolate_idw <- function(variable, idp_value, plot_data_sp, sedes_municipais_sp) {
  # Criar o objeto gstat para a variável de interesse
  gstat_obj <- gstat::gstat(formula = as.formula(paste(variable, "~ 1")),
                            locations = plot_data_sp,
                            nmax = 15,    # Número máximo de vizinhos para considerar
                            set = list(idp = idp_value))
  
  # Interpolação usando IDW
  interpolated <- predict(gstat_obj, newdata = sedes_municipais_sp)
  
  # Extraindo as coordenadas do objeto Spatial
  coords <- coordinates(interpolated)
  
  # Convertendo para data frame e adicionando as coordenadas
  result_df <- as.data.frame(interpolated)
  result_df$x <- coords[, 1]
  result_df$y <- coords[, 2]
...
# Lista das variáveis para interpolação
variables <- c("rainfall", "idf_a", "idf_b", "idf_c", "idf_d")

# Criação de uma lista para armazenar os resultados
results <- list()

# Iteração para cada valor de Tr_values
unique_tr_values <- unique(plot_data_sf$Tr_values)

for(tr_value in unique_tr_values) {
  # Definir um valor específico de idp para o Tr_value atual
  idp_value <- 2.5  # ou algum outro valor ajustável
  
  # Filtrar os dados para o Tr_value específico
  plot_data_sp_filtered <- plot_data_sp[plot_data_sf$Tr_values == tr_value, ]
  
  # Interpolar todas as variáveis
  interpolated_data <- lapply(variables, function(var) {
    interpolate_idw(var, idp_value, plot_data_sp_filtered, sedes_municipais_sp)
  })
  
  # Combinar os resultados em um único data frame
  combined_results <- do.call(cbind, interpolated_data)
  combined_results$Tr_value <- tr_value
...
# Converter os resultados para um objeto sf
final_results_sf <- st_as_sf(final_results, coords = c("x", "y"), crs = 4326)
...
# Função para realizar a interpolação IDW com um valor de idp específico
interpolate_idw <- function(variable, idp_value, plot_data_sp, sedes_municipais_sp) {
  # Criar o objeto gstat para a variável de interesse
  gstat_obj <- gstat::gstat(formula = as.formula(paste(variable, "~ 1")),
                            locations = plot_data_sp,
  ...
  # Retornar o resultado como um data frame
  return(as.data.frame(interpolated))
}
...
for(tr_value in unique_tr_values) {
  # Definir um valor específico de idp para o Tr_value atual (você pode ajustar isso conforme necessário)
  idp_value <- 2.5  # ou algum outro valor ajustável
...
interpolate_idw <- function(variable, idp_value, plot_data_sp, sedes_municipais_sp) {
  ...
  # Interpolação usando IDW
  interpolated <- gstat::idw(gstat_obj, newdata = sedes_municipais_sp)
